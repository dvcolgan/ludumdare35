...
module.exports = types.checkClass(Engine = (function() {
  ...
  Engine.prototype.create = function() {
    ...
    return this.player.body.bounce.y = 0.2;
  };

  Engine.prototype.loadMap = function() {
    var col, i, len, level, ref, results, row, rowData, tile, tileStr;
    level = levels[this.roomCol + "x" + this.roomRow];
    ...
    for (row = i = 0, len = ref.length; i < len; row = ++i) {
      ...
      results.push((function() {
        ...
        for (col = j = 0, len1 = ref1.length; j < len1; col = ++j) {
          ...
          results1.push(this.tilemap.putTile(tile, col - 1, row - 1));
        }
        return results1;
      }).call(this));
    }
  ...
  Engine.prototype.update = function() {
    if (this.player.x < 0) {
      this.player.x = SCREEN_WIDTH - this.player.body.width;
      this.roomCol--;
      this.loadMap();
    }
    if (this.player.x > SCREEN_WIDTH - this.player.body.width) {
      this.player.x = 0;
      this.roomCol++;
      this.loadMap();
    }
    if (this.player.y < 0) {
      this.player.y = SCREEN_HEIGHT - this.player.body.height;
      this.roomRow--;
      this.loadMap();
    }
    if (this.player.y > SCREEN_HEIGHT - this.player.body.height) {
      this.player.y = 0;
      this.roomRow++;
      this.loadMap();
    }
